...
function extractObject(obj, mapping) {
    var mapped = {}; // to => from
    _.each(mapping || {}, function(from, to) {
        if (_.isNull(from) || _.isNull(to)) return; // skip
        var value = _.get(obj, from);
        if (!_.isUndefined(value)) _.set(mapped, to, value);
    });
    return mapped;
};

function formatKeys(obj, callback) {
...
function normalizeQuery(obj) {
    return formatKeys(obj, function(key) {
        if (key === 'id' && String(_.get(obj, key)).indexOf('@') > 0) {
            key = 'email';
        } else if (key === 'id') {
            key = 'user_id';
        ...
        switch (key) {
            ...
            case 'or':
                ...
                if (_.isArray(value) && value.length === 1 && parentKey) {
                    ...
                    search.push('(' + _.map(value, buildQuery).join(logic) + ')');
                }
                break;
            case 'gt':
                var v = value + 1;
            ...
            case 'nin':
                ...
                if (parentKey && conditions.length > 0) {
                    conditions = _.map(conditions, function(v) {
                        return buildSubQuery(parentKey, v);
                    });
                    q = buildQuery({ or: conditions }, key);
